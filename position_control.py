...
class Controller(object):
    ...
    def __init__(self):
        ...
        self.fresh_x = False
        self.fresh_y = False
        self.setpoint_pub_x = rospy.Publisher(
            'setpoint_x', Float64, queue_size=5, latch=True)
        self.setpoint_pub_y = rospy.Publisher(
            'setpoint_y', Float64, queue_size=5, latch=True)
        self.velocity_pub = rospy.Publisher('mufasa/cmd_vel', Twist, queue_size=10)
        # Initialize publishers to pid_enable topics
        self.pid_x_pub = rospy.Publisher('enable_x', Bool, queue_size=10)
        self.pid_y_pub = rospy.Publisher('enable_y', Bool, queue_size=10)
        self.velocity = Twist()
        # Initialize subscriber that listens to output of PIDs
        self.control_sub_x = rospy.Subscriber(
            'control_x', Float64, self.control_callback_x)
        self.control_sub_y = rospy.Subscriber(
            'control_y', Float64, self.control_callback_y)
        # Initialize subsriber to pose topic
        self.pose_sub = rospy.Subscriber('pos', String, self.pose_callback)
    ...
    def position_control_service(self, req):
        ...
        self.pid_x_pub.publish(True)
        self.pid_y_pub.publish(True)
    ...
    def pose_callback(self, pos):
        pose = ast.literal_eval(str(pos)[7:-21])
        dist = numpy.linalg.norm(pose)
        rospy.loginfo("Got distance: %s", dist)
        if (dist <= 0.05):
            self.set_velocity = False
            self.pid_x_pub.publish(False)
            self.pid_y_pub.publish(False)
            null_vector = Twist()
            self.velocity_pub.publish(null_vector)
